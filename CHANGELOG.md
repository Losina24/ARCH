...
### Added
- A new "Agents" tab in the run detail view shows what every agent is doing right now, all at once: the Architect explicitly listed as "Waiting" between reviews and consultations (no separate action needed to see that — it's the same status Monitor already derived from the absence of an activity event), and every Worker's live tool/file activity, interleaved into one chronological, attributed, auto-following feed instead of requiring you to pick one agent at a time in Console.
